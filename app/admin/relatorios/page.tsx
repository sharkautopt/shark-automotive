...
export default async function AdminRelatoriosPage() {
  ...
  const kpis = [
    { label: "Valor em Stock", value: formatEuro(data.stockValue), icon: Euro, color: "text-primary", bg: "bg-primary/10" },
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <main className="flex-1 p-8 ml-64">
        <div className="space-y-8">
          {/* Header */}
          <div>
            <h1 className="font-display text-4xl text-foreground">RELATÓRIOS</h1>
            <p className="text-muted-foreground/60 mt-1">Análise de desempenho do negócio</p>
          </div>

          {/* KPIs */}
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {kpis.map((kpi, i) => (
              <div key={i} className="bg-secondary/30 border border-primary/10 rounded-xl p-6">
                <div className={`w-12 h-12 ${kpi.bg} rounded-lg flex items-center justify-center mb-4`}>
                  <kpi.icon className={`w-6 h-6 ${kpi.color}`} />
                </div>
                <p className="text-muted-foreground/60 text-sm font-mono">{kpi.label}</p>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="bg-secondary/30 border border-primary/10 rounded-xl p-6">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h2 className="font-display text-2xl text-foreground">DISTRIBUIÇÃO DE STOCK</h2>
              </div>
            ...
            <div className="bg-secondary/30 border border-primary/10 rounded-xl p-6">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h2 className="font-display text-2xl text-foreground">LEADS POR TIPO</h2>
              </div>
              <div className="space-y-4">
                {Object.entries(data.leadsByType).length > 0 ? (
                  Object.entries(data.leadsByType).map(([type, count]) => (
                    <div key={type}>
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-muted-foreground/80">{typeLabels[type] || type}</span>
                        <span className="text-foreground font-mono">{count}</span>
                      </div>
                      <div className="h-2 bg-background rounded-full overflow-hidden">
                        <div className="h-full bg-primary rounded-full transition-all" style={{ width: `${(count / maxTypeCount) * 100}%` }} />
                      </div>
                    </div>
                  ))
                ) : (
                  <p className="text-muted-foreground/50 text-center py-4">Sem dados</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-secondary/30 border border-primary/10 rounded-xl p-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="font-display text-2xl text-foreground">PIPELINE DE LEADS</h2>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              {Object.keys(statusLabels).map((status) => (
                <div key={status} className="bg-background/50 rounded-lg p-4 text-center">
                  <p className="font-display text-3xl text-foreground">{data.leadsByStatus[status] || 0}</p>
                  <p className="text-muted-foreground/50 text-xs font-mono mt-1">{statusLabels[status]}</p>
                </div>
              ))}
            </div>
          </div>
