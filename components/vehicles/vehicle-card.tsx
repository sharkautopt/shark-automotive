...
export function VehicleCard({ vehicle, featured = false, className }: VehicleCardProps) {
  ...
  return (
    <motion.div
      className={cn(
        'group relative bg-card overflow-hidden border border-border/50 hover:border-border transition-all duration-500',
    ...
    >
      <Link href={`/inventario/${vehicle.id}`} className="block">
        ...
        <div className={cn(
          ...
          {/* Sold navy tint */}
          {isSold && (
            <div className="absolute inset-0" style={{ backgroundColor: 'rgba(10, 17, 28, 0.35)' }} />
          )}
          ...
          {vehicle.status !== 'available' && (
            <div className="absolute top-4 left-4">
              <span className={cn(
                ...
                vehicle.status === 'reserved' && 'bg-background text-muted-foreground border border-border',
                vehicle.status === 'sold' && 'bg-background text-foreground border border-border'
              )}>
        ...
        <div className="p-5">
          ...
          <div className="grid grid-cols-4 gap-2 pt-4 border-t border-border/50">
            ...
            <div className="flex flex-col items-center text-center min-w-0">
              <Settings2 className="w-4 h-4 text-primary mb-1" />
