  ...
  return (
    <section className="border border-border bg-card">
      <header className="border-b border-border px-6 py-4">
        <h2 className="font-display text-xl tracking-wide text-foreground">Facturas</h2>
      </header>

      {invoices.length === 0 ? (
        <p className="px-6 py-8 text-sm text-secondary-foreground">Sem facturas disponíveis.</p>
      ) : (
          ...
          <table className="w-full min-w-[560px] text-left">
            ...
            <tbody className="divide-y divide-border">
              {invoices.map((inv) => {
                ...
                return (
                  <tr key={inv.id} className="text-sm text-foreground">
