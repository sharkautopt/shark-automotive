...
-- --------------------------------------------------- import requests (INT-11)
-- Public "Encomenda" page: simulator submissions + search requests.
-- The URL/paste modes feed an extraction job; `extracted` holds its output.
create type import_input_mode as enum ('url', 'paste', 'manual');

create table import_requests (
  id            uuid primary key default gen_random_uuid(),
  lead_id       uuid references leads on delete set null,
  mode          import_input_mode not null,
  raw_input     text,                        -- pasted advert text or source URL
  extracted     jsonb,                       -- {make, model, year, cc, co2, fuel, price…}
  isv_estimate  numeric(10,2),
  total_estimate numeric(10,2),
  created_at    timestamptz not null default now()
);
create index on import_requests (created_at desc);

alter table import_requests enable row level security;
create policy "staff read import_requests" on import_requests
  for select using (is_staff());
create policy "anon insert import_requests" on import_requests
  for insert with check (true);

-- Partner enquiries (Shark Partners page) land in `leads` with
-- source='parceiros'; capital_interest carries the amount field.
alter table leads add column if not exists capital_interest numeric(12,2);
