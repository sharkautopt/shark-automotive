...
`Shark Automotive Site.dc.html` is the public site: **six pages in one file**,
switched by `state.pg`. In production each is a real route.

| `state.pg` | Route | Content |
| --- | --- | --- |
| `inicio` | `/` | Hero + live cost estimator, serviço, dossier, fotografia, FAQ, CTA |
| `viaturas` | `/viaturas` | Inventory grid (INT-02) + sob-encomenda card |
| `encomenda` | `/encomenda` | 6-step process, import simulator (INT-11), request form |
| `parceiros` | `/parceiros` | Shark Partners: two participation models, example operation, enquiry form |
| `quemsomos` | `/quem-somos` | **Not written** — copy pending from client |
| `contacto` | `/contacto` | Dossier request + WhatsApp |

The `<sc-if>` blocks map 1:1 to pages; the header nav and footer nav are the
same component on every page.

### What the public site needs wired
| Code | Where | What |
| --- | --- | --- |
| INT-02 | Viaturas | `vehicles` where `published`; `featured` first |
| INT-04 | Viaturas | photos from `vehicle-photos` |
| INT-07 | Contacto / Encomenda / Parceiros | insert into `leads` with `source` = `contacto` / `importacao` / `parceiros`; the Parceiros form's capital field → `leads.capital_interest` |
| INT-08 | every page | `site_content` per page + block |
| INT-09 | Início calculator | `settings` supplies `comissaoFixa`, `custoTransporte`, `custoLegalizacao` — currently DC props |
| INT-10 | Viaturas | "Descarregar ficha" → signed URL for the generated ficha de vidro |
| INT-11 | Encomenda simulator | `import_requests`; URL and paste modes need a server-side extraction job (the client must never call an AI provider directly) |

**No form on the public site is wired.** Each needs validation, spam protection,
the insert, and a confirmation state. The ISV figures the calculator shows are
estimates — see Open question 5.

### Colour and type system (current)
The public site and admin now share the brand system below, which **supersedes
the petrol/chalk + Anton/IBM Plex tokens listed above** in the files delivered
after this note:

| Token | Hex | Use |
| --- | --- | --- |
| Navy | `#0A111C` | Base background |
| Navy deep | `#060C15` | Page background, footers |
| Surface | `#0F1826` | Raised panels |
| Surface alt | `#131C2B` | Hover, inset tiles |
| Cyan | `#2F80ED` | Accent — 1–2 elements per section only |
| White | `#FFFFFF` | Headings |
| Grey light | `#94A0B0` | Secondary text |
| Grey dark | `#6B7280` | Labels, fine print |
| Divider | `#3A4254` | Hairlines |

Typography: **Inter** throughout — 900 headings (uppercase, `-0.02em`), 600
labels and buttons, 500 body, 400 secondary. Georgia only for legal/document
